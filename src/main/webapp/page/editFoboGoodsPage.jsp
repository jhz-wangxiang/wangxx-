    ...
    <div class="left content0718">
    	<div class="column-content0718 goods-manager0719">
    		...
    		<form method="post" class="clearfix" id="baseForm" data-options="novalidate:true">
	    		<div class="baseinput"><p class="text_right l">商品标题：</p><input type="text" name="productName" id="productName" class="editinput_0216 l" /><p class="l">编号：自动生成</p><div class="vspace"></div></div>
	    		<div class="baseinput"><p class="text_right l">商品描述：</p><textarea id="describe" class="describe editinput_0216 edittextarea_0216 l" ></textarea><div class="vspace"></div></div>
	    		<div class="baseinput"><p class="text_right l">重量：</p><input name="weight" id="weight" type="text" class="editinput_0216 l" /><p class="l">kg</p><div class="vspace"></div></div>
	    		<div class="baseinput"><p class="text_right l">价格：</p><input name="productPrice" id="productPrice" type="text" class="editinput_0216 l"  /><p class="l">金币</p><div class="vspace"></div></div>
	    		<div class="baseinput"><p class="text_right l">库存：</p><input name="balance" id="balance" type="text" class="editinput_0216 l" /><p class="l">件</p><div class="vspace"></div></div>
...
var editFobo_fn = {
	...
	productParms_fn:function(productId){
		$.ajax({
			...
			success:function(json){
                ...
                if(json[0].status==2){
                	$('#issuedDateStr').datetimebox('setValue',Common.getLocalTime(json[0].issuedDate));
                }
			}
		});
	},
	...
	releaseSub:function (){
	    /*商品标题，非法字符，30个汉字，其他的非空校验，重量，价格，库存正整数校验*/
	    ...
	    if(validateRules.isNull(ltrim(productName)) || pattern.test(productName) || productName.replace(/[^\x00-\xff]/g, "**").length>200 ){
	        $.messager.alert('提示信息','商品标题不能为空，长度不能超过200位，且不能包含非法字符','warn');
	        return false;
		}
		if(validateRules.isNull(ltrim(describe)) ||  describe.replace(/[^\x00-\xff]/g, "**").length>200){
            $.messager.alert('提示信息','商品描述不能为空，长度不能超过200位','warn');
            return false;
		}
		var weight2=weight.replace(".",   "");
		if(validateRules.isNull(ltrim(weight)) || !new RegExp("^[0-9]*$").test(weight2) || weight.length>10){
            $.messager.alert('提示信息','商品重量不能为空，且只能为数字，长度不能超过10位','warn');
            return false;
		}
		if(validateRules.isNull(ltrim(productPrice)) || !new RegExp("^[0-9]*$").test(productPrice) || productPrice.length>6){
            $.messager.alert('提示信息','商品金币不能为空，且只能为正整数，长度不能超过6位','warn');
            return false;
		}
        if(validateRules.isNull(ltrim(balance)) || !new RegExp("^[0-9]*$").test(balance) || balance.length>10){
            $.messager.alert('提示信息','商品库存不能为空，且只能为正整数，长度不能超过10位','warn');
            return false;
        }
		...
		var obj={"productId":$('input[name="productId"]').val(),"productName":productName,"describe":describe,"weight":weight,"productPrice":productPrice,"balance":balance,"status":status,"sort":sort};
		if(status==2){
