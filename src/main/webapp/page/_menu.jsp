		...
		<dl id="menu-count">
			...
			<dd <c:if test="${param.classify=='count'}">style="display: block;"</c:if>>
				<ul>
					...
					<%if(munuId.contains("12")){%><li <c:if test="${param.level=='count_total'}">class="current"</c:if>><a href="<%=basePath%>v1/page/countTotal" title="总表统计">总表统计</a></li><%} %>
				</ul>
			</dd>
		</dl>
		<%} %>
