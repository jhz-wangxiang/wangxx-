 ...
 */
var LogisticsPaymentManagerPage = function () {
    var columnsArr = [
{
    ...
    formatter: function (value, row, index) {
        ...
        if(value==1){
            ...
            /*<a class="btn detail-btn0719 marRig10" href="javascript:;" onclick="LogisticsManagerSearch.copyLogisticsWindow(' + row.id + ',\'pay\')">拷贝</a>*/
            ...
            if(userName!='15321792627' && userName!='dingdanzhuanyuan'){
                val=val+'<a  class="btn detail-btn0719 marRig10" href="' + rootPath + 'v1/lLogisticsManagerController/editlogisticsPaymentManagerPage?detailId=' + row.id + '">编辑</a>';
            }
    ...
    var exportFileByForm=function () {
        $('#formBtn0424').click(function () {
           $(this).attr('href','javascript:;');
            ...
            $(this).attr('href',rootPath+'v1/exportLLogis/exportPaymentLLogisAll?'+$('#searchForm').serialize());
        });
            ...
            if (selRows.length == 0) {
                ...
                $('#exportExcelWin').dialog({
                    ...
                    buttons: [{
                        ...
                        width: '80px',
                        handler: function () {
                            $(this).attr('href', rootPath + 'v1/exportLLogis/exportPaymentLLogis?ids=' + arr);
                            $('#exportExcelWin').dialog('close');
                            $('#order').datagrid('uncheckAll');
                        }
                    }]
    ...
    var openHeaderWin=function () {
        $(".table-requirement-btn").click(function(){
            ...
            if(Common.getCookie('payment_header')!=null){
                var orderLen=Common.getCookie('payment_order');
                var wuliuLen=Common.getCookie('payment_wuliu'),moneyLen=Common.getCookie('payment_money'),addressLen=Common.getCookie('payment_address'),definedLen=Common.getCookie('payment_defined');
                ...
                var headerArr=Common.getCookie('payment_header').split('_');
    ...
    var lotUpdateInit=function () {
        $('#lotUpdateBtn').click(function () {
            ...
            if(selRows.length==0){
                ...
                $(this).attr('href',rootPath+'v1/lLogisticsManagerController/editlogisticsPaymentManagerPage?detailId='+ids+'&f=l');
            }
        });
    };
    return {
        init: function () {
            ...
            LogisticsPaymentManagerPage.getCookieFun();
        ...
        getCookieFun:function () {
            if(Common.getCookie("payment_header")!=null){
                var getCookie = Common.getCookie("payment_header").split("_");
                $('input[name="order0407"]').val(Common.getCookie('payment_order'));
                $('input[name="wuLiu0407"]').val(Common.getCookie('payment_wuliu'));
                $('input[name="money0407"]').val(Common.getCookie('payment_money'));
                $('input[name="address0407"]').val(Common.getCookie('payment_address'));
                $('input[name="defined0407"]').val(Common.getCookie('payment_defined'));
                var selColumnsArr = [];
     ...
     * */
        importExcel: function (obj) {
            Common.loading_mask();
            $.ajaxFileUpload({
                url: rootPath + 'v1/exportLLogis/importPaymentLLogistManaExcel?'+ Math.random(),
                secureuri: false,
                fileElementId: 'file',
                dataType: 'string',
                success: function (res) {
                    ...
                    if (res.resultCode == 'FAIL') {
                        ...
                        if (res.illegalList.length != 0) {
                            html.push('<p style="line-height: 22px;font-size:14px; color: red;">非法订单' + res.illegalNum + '条数：<a href="javascript:;" class="easyui-linkbutton0410" onclick="LogisticsPaymentManagerPage.searchIllegalOrder(\'' + res.illegalListOrder.join(',') + '\')">查看非法订单</a></p>');
                            html.push('<ol>');
        ...
        submitTableHeader: function () {
            ...
            $('#table-show-condition-window').window('close');
            Common.setCookie('payment_header',headerArr.join('_'));
            Common.setCookie('payment_order',$('input[name="order0407"]').val());
            Common.setCookie('payment_wuliu',$('input[name="wuLiu0407"]').val());
            Common.setCookie('payment_money',$('input[name="money0407"]').val());
            Common.setCookie('payment_address',$('input[name="address0407"]').val());
            Common.setCookie('payment_defined',$('input[name="defined0407"]').val());
