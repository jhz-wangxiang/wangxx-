...
public class DispatchedController {
	...
	@Resource
	private DispatchedService channelService;
	...
	public JSONObject getChannelListPage(Dispatched record, Integer pageNumber, Integer pageSize) throws Exception {
		...
		String result = "";
		rows = channelService.getDispatchedListPage(record, pageNumber, pageSize);
	...
	@ResponseBody
	public Map<String, Object> deleteDispatched(Dispatched record) throws Exception {
		int result = -1;
		record.setExp1("0");
		result = channelService.updateByPrimaryKeySelective(record);
		if (result == 0) {
